...
//setup configuration ~typically done in the backend
// in a full stack app -- save your api keys in a dot env file - read it up!
// it is a security risk to place your api key here like this.
const APIKey = "#";
//from documentation
const imgApi = "https://image.tmdb.org/t/p/w1280";
const searchUrl = `https://api.themoviedb.org/3/search/movie?api_key=${APIKey}&query=`;

//get form element from html (the search)
const form = document.getElementById("search-form");
// get user search input
const query = document.getElementById("result");

// container where results will be published
...
async function fetchData(url) {
  try {
    const response = await fetch(url); //Promise must wait until url is fetched
    if (!response.ok) {
      //throw exception
...
//create dynamic movie display cards
function createMovieCard(movie) {
  ...
  const { poster_path, orginal_title, release_date, overview } = movie;

  const imagePath = poster_path ? imgApi + poster_path : "./img-01.jpeg"; //ternary operator condition ? expressiontrue : expressionfalse

  //check if title is 15 characters to fit. If it isn't, then cut it off and add the ...
...
//innerHTML will allow you to change what is in the respective HTML element based on id
...
function showResults(item) {
  //grab the card from the createMovieCard function
  const newContent = item.map(createMovieCard).join("");
  result.innerHTML += newContent || "<p> No Results Found. Search again. </p>";
}

//Load more results. for when the user reaches bottom of page
...
//detect end of page and load more results. will call loadMoreResults
function detectEnd() {
...
//input param - event param
async function handleSearch(e) {
  console.log("debug inside handleSearch function...");
  //prevent form from restting when submit is clicked
  e.preventDefault();
...
// not - while specifying/calling the function here, we do not include the first brackets
//when user submits, do this. when user scrolls, do this
